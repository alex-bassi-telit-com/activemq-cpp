--------------------------------------------------------------------------
ActiveMQ CPP Library (Embarcadero C++ Builder ready)
--------------------------------------------------------------------------

ActiveMQ CPP is a messaging library that can use multiple protocols to
talk to a MOM (e.g. ActiveMQ). Inside this fork you can find also C++ Builder
project files to build the library from the IDE.

1 Dependencies 
--------------------------------------------------------------------------
1.1 C++ Builder specific info

To build the library with C++ Builder you only need Apache Runtime Library 
to be present in a specific folder.

Please also note that at the current time the library tests cannot be build 
using this tool.

Expected Folder tree:
		<activemq cpp repository root folder>\activemq-cpp\
		remote_apr\apr_build (containing the .lib)
		remote_apr\apr_source\include (include files)

Before building the activemq library you need to compile APR library, then 
copy the apr.h file generated by the build system	from apr\apr_build to 
apr\apr_source\include.


1.2 Other toolchains dependencies.
...
-----------------------------
Note from fork administrator:
Although this fork is specifically designed to compile using C++ Builder (version 10.2.3 
or major) there are no modifications to the original sources, therefore you
can build the library using all supported toolchains by following the original 
indications.
-----------------------------------------------------------------------------
